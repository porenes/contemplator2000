...
function preload() {
  ...
  aliendatacenter = loadImage("img.jpg");
...
function setup() {
  ...
  msg = createP(
    "Upload a <b>portrait</b> preview of your artwork </br>that is at 4:5 aspect ratio"
  );
  msg.position(20, 1000);

  input = createFileInput(handleFile);
  input.position(20, 1075);
  let c = createCanvas(1300, 700);
  c.id("mycanvas");
  ...
  input6 = createElement(
    "textarea",
    "Lorem ipsum dolor sit amet, consectetur adipiscing elit. \nPhasellus massa arcu, convallis eu tristique a, euismod a purus. \nIn hac habitasse platea dictumst. \nUt sed ..."
  );
...
function draw() {
  background(0);
  //default image
  image(aliendatacenter, 30, 22, 482, 655);
  //image(aliendatacenter, 650, 26, 600, 750);

  //input img
  if (img) {
    image(img, 30, 22, 482, 655);
  }
  ...
  //draw hen logo
  // image(hen, 553, 0, 300, 170);

  // display text from Artwork Name input
  textSize(40);
  text(input2.value(), 600, 230);
  textStyle(BOLD);
  fill(255);

  //display text from Artist Name input
  textSize(22);
  text(input3.value(), 600, 180);
  textStyle(BOLD);
  fill(255);

  //DESCRIPTION Counter Input
  textSize(20);
  text(input6.value(), 600, 330);
  textStyle(BOLD);
  fill(255);

  //OBJKT counter Input
  textSize(30);
  text(input4.value(), 600, 630);
  textStyle(BOLD);
  fill(255);

  //EDITIONS Counter Input
  textSize(30);
  text(input5.value(), 600, 597);
  textStyle(BOLD);
  fill(255);

  //LINK Input
  textSize(16);
  text(input7.value(), 600, 653);
  textStyle(BOLD);
  color(255);
}
